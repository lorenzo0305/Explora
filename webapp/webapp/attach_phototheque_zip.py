...
# ---------------- Tokens & recouvrement lexical ----------------

# stopwords très basiques pour éviter les faux positifs sur les mots-outils
FR_STOPWORDS: Set[str] = {
    "le","la","les","un","une","des","du","de","d","l","au","aux","à","a","en",
    "et","ou","pour","par","sur","sous","chez","dans","près","avec","sans",
    "deux","trois"  # nombres courants (peu discriminants)
}

def tokens_for_overlap(norm_text: str) -> Set[str]:
    """
    Découpe une chaîne déjà normalisée (norm_txt) en tokens,
    filtre les très petits mots et stopwords.
    """
    toks = re.findall(r"[a-z0-9]+", norm_text)
    return {t for t in toks if len(t) >= 3 and t not in FR_STOPWORDS}


def shared_token_count(a_norm: str, b_norm: str) -> int:
    A = tokens_for_overlap(a_norm)
    B = tokens_for_overlap(b_norm)
    return len(A & B)
...
def composite_score(photo: dict, name_norm: str, loc_norm: str) -> Tuple[float, dict]:
    """
    Renvoie (score, breakdown).
    Désormais on impose aussi un recouvrement lexical >= 2 mots identiques
    entre le titre de la photo et le label de l'activité.
    """
    ...
    # 0bis) Recouvrement lexical (nouvelle contrainte)
    overlap = shared_token_count(photo.get("title_norm", ""), name_norm)
    if overlap < 2:
        return 0.0, {"reason": "lexical_overlap_lt2", "overlap": overlap}

    # 1) Exact match prioritaire
    if exact_title_city_match(photo, name_norm, loc_norm):
        return 100.0, {"exact": 100.0, "overlap": overlap}
    ...
    return min(100.0, float(score_raw)), {"joint": joint, "title": t, "overlap": overlap}
...
def main():
    ...
    for doc in objs:
        ...
        # Scoring précis composite + contrainte d'overlap >= 2
        scored = []
        for _, fast_score, choice_idx in candidates:
            p = photos[found_idx[choice_idx]]
            s, br = composite_score(p, name_norm, loc_norm)
            # Conserver le meilleur des deux pour robustesse, mais seulement si overlap >= 2
            if br.get("reason") == "lexical_overlap_lt2":
